...
function ChildComponent(props: {name: string}) {
  // props.name = '홍길남'; - Error(readonly 속성)
...
function MultiProps({name, colorProps}: MultiPropsType) {
  // {name, age}의 매개변수에
  // 데이터 person을 전달 (person = {name: '홍길동', age: 50})
  ...
  return (
    <div>
      ...
      <Wraaper>
        ...
        <ChildComponent name='홍길서' />
      </Wraaper>

      {/* 여러개의 props 전달 */}
      <MultiProps name='홍길남' colorProps='pink' />
      {/* ...props가 구조 분해 할당을 사용하여 각각의 속성명을 찾아감 */}
      <MultiProps {...props}/>

      <MultiProps colorProps='blue' />
    </div>
  )
}
