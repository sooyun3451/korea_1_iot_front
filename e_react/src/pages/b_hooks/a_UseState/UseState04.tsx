...
3. 상태 관리
: 입력값은 객체로 아나의 useState를 통해 상태 관리 (formData)
...
function UseState04() {
  ...
  const handleSignUpSubmit = (e: React.FormEvent) => {
    ...
    //? 임시 오류 메시지 객체 생성
    // >> 아이디, 비민번호, 이메일 순으로 오류 메시지를 담아두는 객체
    ...
    //* == 아이디 유효성 검사 ==
    if(!id) { // 아이디 입력 필드가 비어 있으면
      tempErrors.id = '아이디를 입력해주세요' // 오류 메시지 설정
      isValid = false;
    }

    //* == 비밀번호 유효성 검사 ==
    if(!password) { // 비밀번호 입력 필드가 비어 있으면
      tempErrors.password = '비밀번호를 입력해주세요' // 오류 메시지 설정
      isValid = false;
    }

    //* == 이메일 유효성 검사 ==
    if(!email) { // 이메일 입력 필드가 비어 있으면
