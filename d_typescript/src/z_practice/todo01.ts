...
function addTodo(todos: ITodoItem[], task: string): ITodoItem[] {
  ...
  // 스프레드 연산자(...) >> 안에 있는 값만 가져옴 []를 제외하고
  // ...todos { id: 2, task: '자바스크립트 복습', completed: false }
  // todos [ { id: 2, task: '자바스크립트 복습', completed: false } ]
  ...
  */
  ...
  id: Math.max(0, ...todos.map(todo => todo.id)) + 1 , // max 값을 가져와서 +1 현재 max가 3이면 +1해서 이번에 들어가는 값의 id는 4
  task: task,
  completed: false
  };

  //기존 할일 목록에 새로운 할 일 추가
  // : 스프레드 연산자(기존 리스트의 요소 + 새로운 요소 >> 새로운 배열을 생성): 매개변수로 받은 값의 주소와 달라진 주소로 저장
  const newTodos = [...todos, newTodo];
  // 원래있던 todo + 새로 만들어진 Todo 넣기
...
filter((value, index, array) => {조건식을 넣고 true인 것들만 새로운 배열})
>> 모든 요소를 순회하면서 해당 "조건에 맞는 요소만 새로운 배열"로 반환
*/

// let changeTodo; - todo01.ts의 전역변수
...
function completedTodo(todos: ITodoItem[], id: number): ITodoItem[] {
  ...
  const changeTodo: ITodoItem[] = todos.map(todo =>
    ...
    >> 순회하면서 인자로 받아온 id와 같은 todo의 completed를 반대로 넣기 같지않을 경우 그냥 todo return
    */
...
function deleteTodo(todos: ITodoItem[], id: number): ITodoItem[] {
  ...
  // 순회하면서 인자로 받아온 id와 todo의 id가 다른 것들만 return >> id가 같은것은 삭제(된것 같은 효과)
