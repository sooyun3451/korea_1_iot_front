...
var TaskManager = /** @class */ (function () {
    ...
    TaskManager.prototype.renderTasks = function (containerId) {
        ...
        this.tasks.forEach(function (task) {
            ...
            checkbox.checked = task.completed; // completed가 true면 체크된 모양
            checkbox.onchange = function () {
                task.completed = !task.completed;
                _this.renderTasks(containerId);
            };
            // <li>content의 값<button>삭제</button><input type='check-box' checked /></li> 
            if (task.completed) {
    ...
    TaskManager.prototype.updateTaskCount = function () {
        ...
        // countElement.textContent = `할 일 개수: ${this.tasks.length}`; - Error: 'countElement'은(는) 'null'일 수 있습니다.
        // as 타입 선언을 통해 DOM요소의 존재 여부 체킹을 하지 않은 경우
        // : 존재 여부를 조건문으로 검증
        if (countElement) {
...
//# 웹 페이지가 모두 로드되었을떄 실행된 함수
document.addEventListener('DOMContentLoaded', function () {
    // '문자열 타입'의 할 일 관리자 인스턴스 생성
