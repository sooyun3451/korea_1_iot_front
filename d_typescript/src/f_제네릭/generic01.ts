...
// >> 해당 컴포넌트가 처리한 데이터 타입을 미리 지정하지 않고, 해당 컴포넌트를 사용하는 시점에서 원하는 데이터 타입 지정

//# 2. 제네릭 필요성
// - 코드의 중복을 줄임
// - 재사용 가능한 컴포넌트를 생성
// - 타입 안정성 보장
// >> '컴파일 시점'에서 타입을 체크('런타임'환경에서 발생할 수 있는 에러를 방지)
...
// "타입변수"
// : 임의의 키워드를 사용하여 명시
// : 일반적으로 대문자 알파벳 하나를 사용(T: type, U(의미는 없고 그냥 T뒤의 알파벳이라서 사용), ...)
...
let str2 = generic('hello'); // - Error X: 제네릭의 경우 타입 변수에 값을 유추 가능(생략하지 않는 것이 좋음)
// let str3 = generic<string>(3); -Error: 'number' 형식의 인수는 'string' 형식의 매개 변수에 할당될 수 없습니다.
generic<Number>(123); 
...
let pairOuput = pair<string, number>('안녕', 345);
let pairOuput2 = pair<number, string>(345, '안녕');

function pair2<T, U> (first: T, second: T): U { //U를 T로 작성 가능
  return [first, second] as U
}


let pair2Ouput = pair2<string, string[]>('hello', 'hi');
// ['hello','hi']
...
function genericFunc<T>(args: T[]): T[] { // string[] , number[]
  console.log(`배열의 길이: ${args.length}`); // 배열의 길이: 3
  return args;
}
...
console.log(myGenericString.add('홍', '길동')); // 홍길동
