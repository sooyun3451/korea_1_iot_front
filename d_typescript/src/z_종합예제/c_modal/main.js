...
var _this = this;
{
    ...
    var addEventListeners_1 = function (users) {
        var userList = document.getElementById("user-list"); // 여기도 캐스팅이 가능합니다.
        if (userList) {
            userList.addEventListener("click", function (event) {
                var target = event.target; // 타입 캐스팅 추가
                var userCard = target.closest(".user-card"); // 타입 캐스팅 추가
                if (userCard) {
        ...
        var modal = document.getElementById("user-modal"); // 캐스팅 추가
        var closeModal = document.getElementsByClassName("close")[0]; // 캐스팅 추가
        if (modal && closeModal) {
