...
function identifyPerson(person: Person) {
  ...
  if('isAdmin' in person) {
    console.log('Admin 계정입니다');
  }else {
    console.log('User 계정입니다');
  }
}
...
identifyPerson(adminPerson); // Admin 계정입니다
identifyPerson(userPerson); // User 계정입니다
...
type Employee = PersonType & ContantDetails;

let mployee1: Employee = {
  name: '홍길동',
  age: 30,
...
function createCar(vehicle: Vehicle, engine: Engine): Car {
  // 구조분해할당 + 스프레드 연산자를 사용하는 객체 생성 방법
