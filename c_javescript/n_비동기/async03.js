...
myPromise
  ...
  .then((nextresult) => {
    console.log(nextresult); // 다음 수행 시 필요한 데이터
  })
  .catch((error) => {
    console.error(error); // 프로미스 실패.(빨간색으로 나옴)
  })
  .finally(() => {
    console.log('무조건 실행');
  });
...
fetchData()
  ...
  .then(prosessResult => console.log(prosessResult)) //3초
