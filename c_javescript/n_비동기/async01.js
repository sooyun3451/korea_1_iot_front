...
// work함수 호출 시 for문 작업 중 다른 작업을 동시 처리 X
// : for문 완료할 때 까지 end 변수 작업 X
...
//& another함수 작업 완료 후 수행할 코드 설정
// >> 함수를 인자로 전달: 콜백함수

function callbackWork(callbackWork) {
  setTimeout(() => {
    const start = Date.now();
  ...
  callbackWork(); // afterWork함수를 callback이라는 이름으로 전달
}
