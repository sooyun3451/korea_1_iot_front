...
console.log(sum(2342,2342)); // 4684
...
const 변수명 = function(parameter, 매개변수) {
  - 함수 기능 작성
  return 반환값;
}
*/
...
//& 3) 화살표 함수
// : 기존의 function키워드를 화살표(=>)로 대체하여 간결한 함수를 작성
// : 항상 익명 함수로 사용
...
csy(); // 최소윤's job is Developer
console.log(username); // 최소윤
// console.log(job); - Error : job is not defined (지역 변수는 해당 스코프를 벗어나면 사용 불가!!)
// 변수를 name이라고 사용할때 밑줄이 그어지면 특정이름을 정하라는 표시 무엇에 대한 name인지(사용하는데는 지장 X)

//* 함수 선언 방식에 따는 베스트 프렉티스
