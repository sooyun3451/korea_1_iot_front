...
//# 프로젝트 구조
// 1. 할 일 추가(C): 객체를 사용하여 할 일을 저장하고 배열에 객체를 추가
// >> 새로운 할 일(객체)을 목록(배열)에 추가

// 2. 할 일 수정(U): 완료된 할 일의 completed값을 전환(토글, toggle)
// >> 할 일의 완료 상태를 변경

// 3. 할 일 삭제(D): 원하는 할 일을 목록에서 제거

// 4. 할 일 목록 출력(R): 현재 목록의 상태를 콘솔에 출력

// >> 'CRUD' 프로그램의 기본 기능
// Create(생성), Read(출력), Update(수정), Delete(삭제)
