...
class Book {
  // 생성자 함수(메서드)
  // : 클래스 호출시 자동으로 실행되는 함수 >> 객체 생성 시 속성값 초기화
  constructor(id, title, author) {
    // this 키워드 - 해당클래스로 생성되어 사용중인 현재의 객체
    this.id = id;
    this.title = title;
    this.author = author;
...
class Library {
  ...
  displayBooks() {
    ...
    this.books.forEach(book => {
      console.log(`${book.id}: ${book.title} by ${book.author} - ${book.isAvailable ? '대여가능' : '대여불가능'}`);
    })
  ...
  rentBook(id) {
    ...
    // 책이 존재하면
    if(book) {
      book.rent(); // 해당 서적의 returnBook메서드 호출
    }else {
...
busanLibrary.addBook('SQLD공부', '홍길동');
busanLibrary.addBook('JS공부', '홍길서');
busanLibrary.addBook('IoT공부', '홍길남');
busanLibrary.addBook('Java공부', '홍길북');

busanLibrary.displayBooks();

busanLibrary.rentBook(1); // 대여 o
busanLibrary.displayBooks();
busanLibrary.rentBook(1); // 대여 x

const seoulLibrary = new Library(); // 서울 도서관 객체 생성
seoulLibrary.addBook('바나나 먹기','이승아');
