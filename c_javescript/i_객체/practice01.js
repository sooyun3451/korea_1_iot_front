...
console.log(upperCaseFruits); // [ 'BANANA', 'ORANGE', 'GRAPE' ]


//cf) 배열메서드의 콜백함수는 주로 화살표 형태 사용
let arrowFruits = fruits.map(fruits => fruits.toUpperCase());

console.log(arrowFruits); // [ 'BANANA', 'ORANGE', 'GRAPE' ]
...
function combineArrays(arr1,arr2) {
  let result = arr1.map((element, index) => {
    return element + arr2[index]; // 동일한 인덱스 번호를 가진 요소끼리 더해져서 반환 (element = 1,2,3 arr2[index] = 4,5,6)
  });
  return result;
}
...
personCopy.name = '홍길서';
console.log(personCopy); // { name: '홍길서', age: '21', isStudent: true }

console.log(person); // { name: '홍길동', age: '21', isStudent: true }
