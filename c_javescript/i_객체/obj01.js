...
let objectName = {
  ...
  // >> 값의 데이터 형식에는 제한 X (배열, 또다른 객체, 함수 모두 사용 가능)
  // >> 키와 값을 하나의 쌍(멤버)으로 나열, 각각의 쌍은 ,(콤마)로 구분
  // >> 하나의 멤버(키와 값)는 :(콜론)을 분리
...
// 2) 생성자 함수 사용 방식
// : new키워드와 함께 '사용자 정의 생성자 함수'를 사용하여 객체를 생성
// >> 같은 형태(구조)의 객체를 여러 개 생성할 때 유용

// : 함수 내부에서는 this 키워드를 사용해 객체의 속성(프로퍼티) 정의

//? cf) 생성자 함수명을 UpperCamelCase 사용을 권장
...
let person1 = new Human('홍길동', 169);
let person2 = new Human('홍길서', 170);

console.log(person1.name); //홍길동
console.log(person1.height); //169

console.log(person2.name); //홍길서
console.log(person2.height); //170
person2.greet(); //Hello, 홍길서