...
// cf) 참조 자료형
// : 함수,배열,객체 등 (복합 자료형)
// >> 변수에 값이 아닌 '메모리 주소를 저장'하여 '참조'(const를 사용해도 error가 나지 않음)
...
sports.length = 6; 
console.log(sports[3]); // undefined - 배열의 길이를 늘리면 새 요소는 'undefined로 초기화 
console.log(sports);

sports[5] = '배구';
console.log(sports); // [ '축구', '야구', 'basketball', <2 empty items>, '배구' ] - 2개의 undefined
...
//! 1) indexOf() ***
// : 배열의 지정된 요소를 찾고 그 요소의 첫번째 인덱스를 반환 (없으면 -1 반환)
...
// >> 수정된 배열의 길이를 반환(return)
let newLength = fruits.push('Cherry');
console.log(newLength); // 3

// 2) 요소 삭제
// pop(); *****: 배열의 마지막 요소를 제거
// >> 그 제거된 요소를 반환(빈 배열은 undefined를 반환)
